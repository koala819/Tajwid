...
export default function AdminHeader() {
  ...
  return (
    <header className="space-y-4 rounded-lg border border-stone-200 bg-white p-6 shadow-sm dark:border-neutral-700 dark:bg-neutral-800">
      <div className="flex items-center gap-2">
        ...
        <p className="text-xs font-normal uppercase tracking-wider text-amber-700 dark:text-amber-500">
          {t('admin', lang)}
        </p>
      </div>
      ...
      <p className="text-sm text-stone-600 dark:text-stone-300">
        {lang === 'ar' ? 'إدارة نقاط الحكام ونشر النتائج النهائية' : 'Gérez les notes des jurys et publiez les résultats finaux'}
      </p>
    </header>
  );
}
