...
export default function PublishToggle({ notes }: PublishToggleProps) {
  const router = useRouter();
  const allPublished = notes.length > 0 && notes.every((n) => n.publie);
  ...
  return (
    <button
      ...
      className="flex cursor-pointer items-center gap-3 transition-opacity hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-40"
    >
      <span className="text-sm text-stone-600 dark:text-stone-300" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        {t('showOnResultsPage', lang)}
      </span>
      {/* Pilule toggle */}
      <span
      ...
      >
        <span
          className={`absolute top-1 flex h-6 w-6 items-center justify-center rounded-full bg-white shadow transition-[left] ${
            isPublished ? 'left-7 text-amber-700 dark:text-amber-500' : 'left-1 text-stone-500 dark:text-stone-400'
          }`}
        >
          {loading ? (
