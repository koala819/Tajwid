...
type QualifierToggleProps = {
  eleveId: string;
  qualified: boolean;
};

export default function QualifierToggle({ eleveId, qualified: initialQualified }: QualifierToggleProps) {
  ...
  return (
    <button
      ...
      disabled={loading}
      title={qualified ? t('qualifiedYes', lang) : t('qualifiedNo', lang)}
      className="flex cursor-pointer items-center gap-3 transition-opacity hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:cursor-wait disabled:opacity-40"
    >
      <span className="text-sm text-stone-600 dark:text-stone-300" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        {t('qualifiedToggleLabel', lang)}
      </span>
      {/* Pilule toggle — vert si qualifié, gris sinon */}
      <span
        className={`relative inline-flex h-8 w-14 shrink-0 rounded-full transition-colors ${
          qualified ? 'bg-green-600 dark:bg-green-500' : 'bg-stone-200 dark:bg-neutral-600'
        }`}
        aria-hidden
      >
        <span
          className={`absolute top-1 flex h-6 w-6 items-center justify-center rounded-full bg-white shadow transition-[left] ${
            qualified ? 'left-7 text-green-600 dark:text-green-500' : 'left-1 text-stone-500 dark:text-stone-400'
          }`}
        >
          {loading ? (
            <span className="h-3 w-3 animate-pulse rounded-full bg-stone-400" />
          ) : qualified ? (
            /* Étoile pleine = qualifié */
            <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor">
              <path d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
            </svg>
          ) : (
            /* Étoile vide = non qualifié */
